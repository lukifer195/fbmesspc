            ...
            for entry in data["entry"]:
                for messaging_event in entry["messaging"]:
                    ...
                    if messaging_event.get("message"):
                        ...
                        log("RECEIVE: {sender_id}  o>>> {recipient} [Me] :   {text}".format(
                            sender_id=sender_id, recipient=recipient_id, text=message_text))
                        try:
...
def send_message(sender_id, recipient_id, message_text):
    if message_text == '@KtrLoi' or message_text == None:
        ...
        log("SEND: {sender_id} [Me]  o>>> {recipient} :   {text}".format(
            sender_id=sender_id, recipient=recipient_id, text=message_text))
...
def log(msg, *args, **kwargs):  # simple wrapper for logging to stdout on heroku
    try:
        ...
        print("<<{}>> : {}".format(datetime.datetime.now() +
                                   datetime.timedelta(hours=7), msg))
    ...
    if list_keywords == "@KtrLoi":
        ...
        with open('export.json', 'r', encoding='utf-8') as f:
            ...
            for key in list_keywords:                 # trượt key trong list keys
                # print(key.values())
                # convert tuple cho tất cả item trong key
                Keyword = tuple(key.items())
            ...
            if len(keyset) == 1:
                for datalist in export.get('data'):
                    ...
                    # bắt đầu list nhận từng tuple thực thể
                    entities_listtotup = []
                    for entity in entities_list:                            # 1 thực thể = 1 tupple // 1 list thực thể = 1 câu
                        # chuyển items thành tuple mới set dc
                        entities_listtotup.append(tuple(entity.items()))
                    entity_set = set(entities_listtotup)
                    # print("entities: ", entity_set)
                    # ^ tìm thực thể (tuple) khác nhau giữa 2 set
                    diff = keyset ^ entity_set
                    lendiff = len(diff)
                    # chạy len(diff) để tìm câu thiếu hoặc dư 1-4 thực thể
                    if lendiff == 0:
